...
FROM (
    ...
    FROM gold.fact_transactions WHERE invested_amount = 0 
	UNION ALL
    SELECT 'AVG Amount Invested' AS metric_name, AVG(invested_amount) AS metric_value, 7 AS order_column
    FROM gold.fact_transactions --Per Transaction in USD
    UNION ALL
    SELECT 'AVG Amount Withdrawn' AS metric_name, AVG(withdrawal_amount) AS metric_value, 8 AS order_column
    FROM gold.fact_transactions --Per Transaction in USD
    UNION ALL
    SELECT 'Total Clients Count' AS metric_name, COUNT(client_key) AS metric_value, 9 AS order_column
